...
class DiffCode:
    ...
    # either three or four lines with the prefixes ('-', '+', '?'),
    # ('-', '?', '+') or ('-', '?', '+', '?') respectively
    CHANGED = 3
    ...
    def __next__(self):
        ...
        if code == '  ':
            ...
            if not incrementalChange:
                ...
                left_change = incrementalChange['left'] if 'left' in incrementalChange else None
                right_change = incrementalChange['right'] if 'right' in incrementalChange else None
                result['leftchanges'] = left_change
                result['rightchanges'] = right_change
